...
def abrir_analise_dados():
    ...
    if df['genre_ids'].notnull().any():
        ...
        sucesso_por_genero = df_generos.groupby('genre_ids')[['vote_average', 'popularity']].mean().sort_values(by='vote_average', ascending=False).head(5)
        ...
        axs[0, 0].set_title("Sem dados de gênero")

    # 2. Comparação por região (idioma original)
    if 'original_language' in df.columns:
        sucesso_por_idioma = df.groupby('original_language')[['vote_average', 'popularity']].mean().sort_values(by='vote_average', ascending=False).head(5)
        ...
        axs[0, 1].set_title("Sem dados de idioma")
...
tb.Button(app, text="🌓 Alternar Tema", bootstyle="secondary", command=alternar_tema).pack(pady=5)
tb.Button(app, text="🎞️ Assistir Trailer", bootstyle="success", command=abrir_trailer).pack(pady=10)

btn_analise = tb.Button(app, text="📊 Análise de Dados", bootstyle="warning", command=abrir_analise_dados)
btn_analise.place(relx=1.0, rely=1.0, anchor="se", x=-20, y=-20)
